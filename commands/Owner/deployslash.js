    ...
    if (message.author.id != "760227928843223060")
      return message.channel.send({
        embeds: [new MessageEmbed()
          ...
          .setTitle("Only S7NX is allowed to deploy the SLASH-COMMANDS")
          .setDescription(`Go to the [Discord-Server](https://discord.gg/a6mtXgtdGb), open a Ticket and ask for it!`)
        ]
      });
    try {
      ...
      if(loadSlashsGlobal){
        let themsg = await message.reply(`<a:Loading:1007611463294275634> **Attempting to set the Global Slash Commands in \`${client.guilds.cache.size} Guilds\`...**`)
        ...
        let guild = client.guilds.cache.get(guildId);
        let themsg = await message.reply(`<a:Loading:1007611463294275634> **Attempting to set the GUILD Slash Commands in \`${guild.name}\`...**`)
