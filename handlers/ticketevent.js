            ...
            channel.send({
                ...
                collector.on('collect', async b => {
                    if (b?.user.id !== user.id)
                        return b?.reply(`<:no:1007436622733774911> **Only the one who typed ${prefix}help is allowed to react!**`, true)
            ...
            if (([...member.roles.cache.values()] && !member.roles.cache.some(r => cmdroles.includes(r.id))) && !cmdroles.includes(interaction?.user.id) && ([...member.roles.cache.values()] && !member.roles.cache.some(r => adminroles.includes(r ? r.id : r))) && !Array(guild.ownerId, config.ownerid).includes(interaction?.user.id) && !member.permissions.has("ADMINISTRATOR") && !member.roles.cache.some(r => theadminroles.includes(r ? r.id : r))&& !theadminroles.includes(member.id)) {
                return channel.send({
                    ...
                    embeds: [new MessageEmbed()
                        .setColor(es.wrongcolor)
                        .setFooter(client.getFooter(es))
                        .setTitle("<:no:1007436622733774911> You are not allowed to delete this Ticket")
                        .setDescription(`${adminroles.length > 0 ? "You need one of those Roles: " + adminroles.map(role => `<@&${role}>`).join(" | ") + cmdrole.join(" | ") + theadminroles.join(" | ")  : `No Admin Roles Setupped yet! Do it with: \`${prefix}help\` You can also add Ticket only Roles with \`${prefix}setup-ticket\``}`)
            ...
            if (([...member.roles.cache.values()] && !member.roles.cache.some(r => cmdroles.includes(r.id))) && !cmdroles.includes(interaction?.user.id) && ([...member.roles.cache.values()] && !member.roles.cache.some(r => adminroles.includes(r ? r.id : r))) && !Array(guild.ownerId, config.ownerid).includes(interaction?.user.id) && !member.permissions.has("ADMINISTRATOR") && !member.roles.cache.some(r => theadminroles.includes(r ? r.id : r))) {
                return channel.send({
                    ...
                    embeds: [new MessageEmbed()
                        .setColor(es.wrongcolor)
                        .setFooter(client.getFooter(es))
                        .setTitle("<:no:1007436622733774911> You are not allowed to add/remove Users to/from this Ticket")
                        .setDescription(`${adminroles.length > 0 ? "You need one of those Roles: " + adminroles.map(role => `<@&${role}>`).join(" | ") + cmdrole.join(" | ") + theadminroles.join(" | ")  : `No Admin Roles Setupped yet! Do it with: \`${prefix}help\` You can also add Ticket only Roles with \`${prefix}setup-ticket\``}`)
                    ]
                });
            }
            ...
            if (([...member.roles.cache.values()] && !member.roles.cache.some(r => cmdroles.includes(r.id))) && !cmdroles.includes(interaction?.user.id) && ([...member.roles.cache.values()] && !member.roles.cache.some(r => adminroles.includes(r ? r.id : r))) && !Array(guild.ownerId, config.ownerid).includes(interaction?.user.id) && !member.permissions.has("ADMINISTRATOR") && !member.roles.cache.some(r => theadminroles.includes(r ? r.id : r))) {
                return channel.send({
                    ...
                    embeds: [new MessageEmbed()
                        .setColor(es.wrongcolor)
                        .setFooter(client.getFooter(es))
                        .setTitle("<:no:1007436622733774911> You are not allowed to add/remove Roles to/from this Ticket")
                        .setDescription(`${adminroles.length > 0 ? "You need one of those Roles: " + adminroles.map(role => `<@&${role}>`).join(" | ") + cmdrole.join(" | ") + theadminroles.join(" | ")  : `No Admin Roles Setupped yet! Do it with: \`${prefix}help\` You can also add Ticket only Roles with \`${prefix}setup-ticket\``}`)
                    ]
                });
            }
            ...
            if (([...member.roles.cache.values()] && !member.roles.cache.some(r => cmdroles.includes(r.id))) && !cmdroles.includes(interaction?.user.id) && ([...member.roles.cache.values()] && !member.roles.cache.some(r => adminroles.includes(r ? r.id : r))) && !Array(guild.ownerId, config.ownerid).includes(interaction?.user.id) && !member.permissions.has("ADMINISTRATOR") && !member.roles.cache.some(r => theadminroles.includes(r ? r.id : r))) {
                return channel.send({
                    ...
                    embeds: [new MessageEmbed()
                        .setColor(es.wrongcolor)
                        .setFooter(client.getFooter(es))
                        .setTitle("<:no:1007436622733774911> You are not allowed to claim this Ticket")
                        .setDescription(`${adminroles.length > 0 ? "You need one of those Roles: " + adminroles.map(role => `<@&${role}>`).join(" | ") + cmdrole.join(" | ") + theadminroles.join(" | ")  : `No Admin Roles Setupped yet! Do it with: \`${prefix}help\` You can also add Ticket only Roles with \`${prefix}setup-ticket\``}`)
                    ]
                });
            }
        ...
        if(interaction?.guildId && interaction?.isSelectMenu() && interaction?.message && interaction?.message.author.id == client.user.id){
            ...
            if(message.id == settings.messageId && (channelId == settings.channelId || message.channelId == settings.channelId)){
               let index = settings.data.findIndex(v => v.value == values[0]);
                    ...
                    try {
                      ...
                      if(client.setups.has(ticketchannel.id) && client.setups.has(ticketchannel.id, "ticketdata"))
                      {
                        ...
                        if(data.state != "closed" && data.menutickettype == 3){
                          return interaction?.reply({content: `<:no:1007436622733774911> **You already have an Ticket!** <#${ticketchannel.id}>`, ephemeral: true});
                        }
                      }
                    } catch {
                ...
                await interaction?.reply({content: `<a:Loading:1007611463294275634> **Creating your ticket?...** (Usually takes 0-2 Seconds)`, ephemeral: true});
                try {
                ...
                if(settings.claim.enabled){
                    ...
                    .setFooter(client.getFooter(es))
                    .setAuthor(client.getAuthor(`A Staff Member will claim the Ticket soon!`, "https://cdn.discordapp.com/emojis/833101350623117342.gif?size=44", "https://discord.gg/a6mtXgtdGb"))
                ...
                await interaction?.editReply({content: `<a:yes_animated:1007611220842528799> **Your Ticket is created!** <#${ch.id}>`, ephemeral: true});
                })
            }
        }
    })
