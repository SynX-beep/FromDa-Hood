    ...
    client.on("messageCreate", async message => {
        try{
            ...
            if(message.channel.id == chatbot){
              ...
              try{
                fetch(`http://api.brainshop.ai/get?bid=168029&key=E3yhZnYlSI1pzuaP&uid=1&msg=${encodeURIComponent(message)}`)
                .then(res => res.json())
                .then(data => {
                  message.channel.send({content: data.cnt}).catch(() => {})
                });
              }catch (e){
                message.channel.send({content: "<:no:1007436622733774911> AI CHAT API IS DOWN"}).catch(() => {})
              }
            }
        ...
        try{
            ...
            for(const user of [...message.mentions.users.values()]){
                if(client.afkDB.has(message.guild.id + user.id)){
                    await message.reply({content: `<a:worrycry:1007606795088441435> **${user.tag}** went AFK <t:${Math.floor(client.afkDB.get(message.guild.id+user.id, "stamp") / 1000)}:R>!${client.afkDB.get(message.guild.id+user.id, "message") && client.afkDB.get(message.guild.id+user.id, "message").length > 1 ? `\n\n__His Message__\n>>> ${String(client.afkDB.get(message.guild.id+user.id, "message")).substring(0, 1800).split(`@`).join(`\`@\``)}` : "" }`}).then(msg=>{
                        setTimeout(()=>{
                            try{
                                msg.delete().catch(() => {});
                            }catch{  }
